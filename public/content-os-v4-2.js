...
function render(){
  ...
  if(w && w.id!==primaryWorkspaceId && currentPage!=='Configuración'){
    page.innerHTML=`${header(currentPage,`${w.name} · espacio aislado del resto de las marcas.`,`<button class="btn" id="emptyConnectAccount">+ Conectar cuenta</button>`)}
    <div class="module-banner"><div class="module-kicker">${w.type.toUpperCase()}</div><h3>${w.name} todavía no tiene datos conectados.</h3><p>Cuando conectemos su Instagram, este módulo mostrará únicamente el contenido, métricas, Stories, patrones, personaje, leads y ventas de ${w.name}. Los datos de Andrés o de cualquier otro cliente nunca aparecen en este contexto.</p></div>
    <div class="client-structure" style="margin-top:14px"><div class="card workspace-card"><div class="panel-title"><h3>Orden del espacio</h3><span class="workspace-tag">Cliente</span></div><div class="account-stack"><div class="account-box"><div class="platform-icon">◎</div><div class="grow"><strong>Instagram Professional</strong><small>Reels, Posts, Stories e insights propios de esta cuenta</small></div><span class="source-status">Sin conectar</span></div><div class="account-box"><div class="platform-icon">DB</div><div class="grow"><strong>Base histórica independiente</strong><small>Cada fila queda asociada al workspace de ${w.name}</small></div><span class="source-status ok">Aislado</span></div></div></div><div class="card workspace-card"><div class="panel-title"><h3>Qué se separa automáticamente</h3></div><div>${['Contenido','Métricas','Stories','Competencia','Patrones','Ideas','Personaje','Conversaciones','Leads','Seguimientos','Ventas','Revenue'].map(x=>`<span class="chip">${x}</span>`).join('')}</div><div class="isolation-note">El selector superior funciona como contexto global. Cambiar de cliente cambia el universo de datos antes de ejecutar cualquier análisis.</div></div></div>`;
    document.getElementById('emptyConnectAccount')?.addEventListener('click',()=>{currentPage='Configuración';render();});
    return;
  }
